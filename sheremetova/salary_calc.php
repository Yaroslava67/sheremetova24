...
// Функция для проверки и добавления колонки в таблицу
...
// Создаем таблицу если её нет и добавляем все необходимые колонки
try {
    // Сначала проверим существование таблицы
    $stmt = $pdo->query("SHOW TABLES LIKE 'salary_calc'");
    $tableExists = $stmt->rowCount() > 0;
    
    if (!$tableExists) {
        // Создаем таблицу целиком
        $createSql = "CREATE TABLE salary_calc (
            calc_id INT(11) AUTO_INCREMENT PRIMARY KEY,
            employee_id INT(11) NOT NULL,
            period_year INT(4) NOT NULL,
            period_month INT(2) NOT NULL,
            taxable_base DECIMAL(15,2) DEFAULT 0,
            rate_percent DECIMAL(15,2) DEFAULT 0,
        ...
        addColumnIfNotExists($pdo, 'salary_calc', 'taxable_base', 'DECIMAL(15,2) DEFAULT 0 AFTER period_month');
        addColumnIfNotExists($pdo, 'salary_calc', 'rate_percent', 'DECIMAL(15,2) DEFAULT 0 AFTER taxable_base');
        addColumnIfNotExists($pdo, 'salary_calc', 'amount', 'DECIMAL(15,2) DEFAULT 0 AFTER rate_percent');
        addColumnIfNotExists($pdo, 'salary_calc', 'payment_status', "ENUM('pending','paid','overdue') DEFAULT 'pending' AFTER amount");
        addColumnIfNotExists($pdo, 'salary_calc', 'payment_date', 'DATE DEFAULT NULL AFTER payment_status');
    }
} catch(PDOException $e) {
    // Ошибка при создании таблицы или добавлении колонок
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_calc'])) {
    ...
    $rate_percent = !empty($_POST['rate_percent']) ? (float)$_POST['rate_percent'] : 13;
...
<head>
    ...
    <style>
        ...
        .employee-selector select { flex: 2; margin: 0; padding: 10px; }
        ...
        <form method="POST">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" name="rate_percent" id="rate_percent" value="13">
                </div>
            </div>
